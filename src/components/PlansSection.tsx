    ...
    <div className="container">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
        {PLANS.map((plan) => (
          <div
            key={plan.id}
            className={`rounded-lg p-6 transition-brand ${
              plan.highlighted
                ? "bg-gold/10 border-2 border-gold relative"
                : "bg-primary-foreground/5 border border-primary-foreground/10 hover:border-gold/30"
            }`}
          >
            {plan.highlighted && (
            ...
            <Link
              to={`/planes#${plan.id}`}
              className={`block text-center py-3 rounded-full text-sm tracking-wide-brand uppercase transition-brand ${
                plan.highlighted
      ...
      <div className="text-center mt-12">
        <Link
          to="/planes"
          className="text-gold hover:text-gold-light text-sm tracking-wide-brand uppercase transition-brand"
        >
